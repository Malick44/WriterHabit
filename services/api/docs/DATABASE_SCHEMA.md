...
- `services/api/migrations/202606110007_grade3_writing_progress.sql`
- `services/api/migrations/202606110008_submission_revision_drafts.sql`
- `services/api/migrations/202606110009_student_messages.sql`
- `services/api/migrations/202606110010_practice_sessions.sql`
- `services/api/migrations/202606110011_practice_session_evidence.sql`
- `services/api/migrations/202606110012_submission_attachment_metadata.sql`
- `services/api/migrations/202606110013_canvas_artifact_storage.sql`
- `services/api/migrations/202606110014_teacher_profile_dashboard_preferences.sql`
- `services/api/migrations/202606110015_user_onboarding_progress.sql`
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users`                | Public app profile for a Supabase auth user, including safe partial onboarding recovery JSON.                                   | `id` references `auth.users(id)`, unique `email`, role check for `student`, `parent`, `teacher`, `admin`, onboarding progress object check, `users_role_idx`; `202606110001_server_owned_roles.sql` rejects public-client and auth-metadata role changes so role grants stay backend/admin-owned. |
...
| `submission_attachments`      | Bounded metadata for uploaded/photo submission evidence.          | Unique submission/client attachment ID, bounded names/MIME/object paths/excerpts, upload status, owner RLS.   |
...
| `submission_revision_drafts`  | In-progress student revision autosave.                 | One row per submission, FK to student profile, bounded revised text, owner RLS.                                                                                               |
...
| `student_messages`            | Student-visible teacher/coach message thread previews. | FK to student profile, sender kind allow-list, bounded preview, select-visible/admin-manage RLS.                                                                              |
...
| `practice_sessions`       | Student-owned daily practice task completions and bounded practice evidence metadata. | Unique student/skill/task/date, bounded estimated minutes, bounded attachment count/text excerpts, owner RLS. |
